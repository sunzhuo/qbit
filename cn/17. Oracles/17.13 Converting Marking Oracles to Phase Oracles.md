...
在此演示中，您将使用 `ApplyMarkingOracleAsPhaseOracle` 操作的参考实现将标记预言机 `IsSeven_MarkingOracle` 转换为相位预言机。然后，您将将此转换后的预言机与相位预言机 `IsSeven_PhaseOracle` 的参考实现进行比较。您已经在之前的任务中实现了这些预言。
...
该转换器提供了一种方法来实现感兴趣的功能作为标记预言机，然后将其转换为相位预言机，然后可以在量子算法中利用它。
